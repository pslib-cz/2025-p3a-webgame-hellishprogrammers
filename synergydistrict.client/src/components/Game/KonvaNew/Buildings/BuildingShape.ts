...
const getTileOutline = (building: MapBuilding, position: Position): OutlineKey[] => {
    ...
    const relevantEdges = building.edges.filter(
        (edge) => edge.position.x === position.x && edge.position.y === position.y
    );
...
const prepareBuilding = ({
    ...
    tileSize,
    debug = false,
}: PrepareBuildingBitmapParams): PreparedBuildingBitmap | null => {
